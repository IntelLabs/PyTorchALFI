...
from object_detection.quantiles_extract_features_img_plot4 import get_quant_ftrace
...
def subplot_full_acts(ftraces_dict, im, lay, typ, figsz, axs, binsz = 50, no=1, label_list = []):
    ...
    if np.max(img_lay) > 1e10 or np.min(img_lay) < -1e10:
        ...
        axs[lay].axes.set_xticks(axs[lay].axes.get_xticks()[::3])
    if lay == len(axs)-1:
...
def main(argv):
    ...
    ###########################################
    flt_type = "hwfault" #"hwfault" #"Gaussian_noise" , Gaussian_blur, Adjust_contrast #for that put in yaml neurons and per_image
    hw_spec = ["weights", [1,1]] #weights, neurons; bit_pos
    noise_magn = 0.1
    random.seed(10) #make noise the same
    lay = 50
    ###########################################

    set_state(yml_file_path, 'rnd_layer',lay)
    ...
    qu_corr = np.array(quant_dict['corr'][N])
    maxs = [max(abs(n)) for n in qu_corr]
    lays_large = np.where(np.array(maxs) > abs(fault_file[-1])/1000.)[0].tolist()
    lays_small = np.where(np.array(maxs) < abs(fault_file[-1])/1000.)[0].tolist()
    print('large layers', lays_large)
    print('small layers', lays_small)
    print()
    ...
    # print()
    # qu_corr = np.array(quant_dict['corr'][N])
    # # check lays with large values in quant_dict TODO:
    ...
    # # figsz = None
    # y_text = 'Output' + " "*25 + 'Conv2' + " "*25 + 'Conv1' + " "*26 + 'Input'
