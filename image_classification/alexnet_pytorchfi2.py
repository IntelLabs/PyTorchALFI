...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/home/qutub/PhD/git_repos/intel_git_repos/pfd_uet/yolov3_ultra/yolov3.pt', help='model path(s)')
...
def main(argv):
    ...
    dl_attr.dl_dataset_name     = "imagenet"
    dl_attr.dl_transform     = transform

    ranger_file_name = 'AlexNet_bounds_ImageNet_train20p_act'
    ...
    num_faults  = [1]
    fault_files = ["/home/qutub/PhD/git_repos/intel_git_repos/pfd_uet/result_files/result_files_paper/yolov3_ultra_50_trials/neurons_injs/per_image/objDet_20220111-002317_1_faults_[0, 31]_bits/coco2017/val/yolov3_ultra_test_random_sbf_neurons_inj_1_50_1bs_coco2017_fault_locs.bin"]
    fault_files = [None]
    resume_dir  = None
    ...
    for id, _num_faults in enumerate(num_faults):
            if apply_ranger:
                ...
                yolov3_ErrorModel = TestErrorModels_ImgClass(model=model, resil_model=model, resil_name='ranger', model_name=model_name, config_location='default_neurons.yml', \
                    ranger_bounds=np.array(bnds), device=device,  inf_nan_monitoring=True, disable_FI=False, dl_attr=dl_attr, num_faults=_num_faults, fault_file=fault_files[id], \
                        resume_dir=resume_dir, copy_yml_scenario = False)

            else:
                yolov3_ErrorModel = TestErrorModels_ImgClass(model=model, model_name=model_name, dataset='coco2017', dl_attr=dl_attr,\
        config_location='default_neurons.yml', inf_nan_monitoring=False)

            yolov3_ErrorModel.test_rand_ImgClass_SBFs_inj()
