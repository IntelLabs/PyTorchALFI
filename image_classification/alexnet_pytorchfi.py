...
def main(argv):
    ...
    if not any('--' in s for s in argv):
        ...
        for i, args in enumerate(argv):
            if '--' in args:
                argv = argv[i:]
                break
    parser = argparse.ArgumentParser("alexnet_pytorch")
    ...
    ranger_file_name = 'AlexNet_bounds_ImageNet1000_act'
    batch_size = 10
    ...
    # batchsize is set in scenarios/default.yml -> ptf_batch_size
    num_faults = [1, 10]
    fault_files = ["/home/qutub/PhD/git_repos/intel_gitlab_repos/nan_debug/logs/fault_rates_alexnet_1_500.bin", "/home/qutub/PhD/git_repos/intel_gitlab_repos/nan_debug/logs/fault_rates_alexnet_10_500.bin" ]
    ...
    for id, _num_faults in enumerate(num_faults):
        for _resil in resil:
            if apply_ranger:
                ...
                net_for_protection = alexnet
                protected_alexnet, _ = get_Ranger_protection(net_for_protection, bnds, resil=_resil)
                protected_alexnet = protected_alexnet.to(device)

            alexnet_ErrorModel = TestErrorModels(model=alexnet, resil_model=protected_alexnet, model_name=model_name, cuda_device=cuda_device,
                                    dataset='imagenet', store_json=True, resil_method=_resil.__name__, config_location='default_weights.yml')
            alexnet_ErrorModel.test_random_single_bit_flip_inj(num_faults=_num_faults, fault_file=fault_files[id], save_fault_file_dir=save_fault_file_dir)
