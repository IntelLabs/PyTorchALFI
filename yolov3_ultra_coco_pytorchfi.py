...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='/home/fgeissle/ranger_repo/ranger/yolov3/yolov3/yolov3.pt', help='model path(s)')
...
def main(argv):
    ...
    fault_files = ["/home/qutub/PhD/git_repos/intel_git_repos/ranger/result_files/result_files_paper/yolov3_ultra_50_trials/neurons_injs/per_image/objDet_20220110-184651_1_faults_[0, 31]_bits/coco2017/val/yolov3_ultra_test_random_sbf_neurons_inj_1_50_1bs_coco2017_fault_locs.bin"]
    fault_files = [None]
    resume_dir = None
    ...
    for id, _num_faults in enumerate(num_faults):
        for _resil in resil:
            if apply_ranger:
                ...
                yolov3_ErrorModel = TestErrorModels_ObjDet(model=model, resil_model=None, model_name=model_name, dataset='coco2017', dl_sampleN=1000,\
        config_location=yml_file, ranger_bounds=np.array(bnds), device=device,  inf_nan_monitoring=True, disable_FI=False)

            else:
                yolov3_ErrorModel = TestErrorModels_ObjDet(model=model, model_name=model_name, dataset='coco2017', dl_sampleN=1000,\
