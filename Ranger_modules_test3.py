from resiliency_methods.Ranger import Ranger
...
class My_Bottleneck(nn.Module):
    ...
    def __init__(self, bn):
        ...
        mdl_bn.insert(2, nn.ReLU(inplace=True))
        mdl_bn.insert(4+1, nn.ReLU(inplace=True))
        ...
        self.bn_model2 = nn.ReLU(inplace=True) #has to be split of for skip connection
...
class My_Bottleneck_Ranger(nn.Module):
    ...
    def __init__(self, bn, bnds):
        ...
        mdl_bn.insert(3, Ranger(self.Bounds[0]))
        mdl_bn.insert(7, Ranger(self.Bounds[1]))
        self.bn_model1 = nn.Sequential(*mdl_bn)
...
def insert_Ranger(model, target_list, target_type_list, bnds_orig):
    """
    Adds Ranger layers
    ...
    for nr_ind in range(len(target_list)):
        ...
        if tp == nn.ReLU:
            ...
            chld_list.insert(ind + 1 + cnt, Ranger(bnds=sel_bnd))  # add ranger layer AFTER ind, with resp. bound
            ...
            chld_list.insert(ind + 1 + cnt, Ranger(bnds=sel_bnd))  # reuse latest bounds, dont have to change nothing
            cnt += 1
            # print('used bound', sel_bnd)

    model_ranger = nn.Sequential(*chld_list)

    return model_ranger, bnds


def get_Ranger_protection(net, bnds):
    """
    ...
    model, _ = insert_Ranger(model, target_list, target_type_list, bnds) #could give modified bounds as second parameter
...
train_batch_size = 4
test_batch_size = 4
# (train_loader, test_loader, classes) = load_data_cifar(train_batch_size, test_batch_size)
...
# net_for_bounds = net
# name = "AlexNet_bounds_ImageNet1000_act" #name of saved file
...
# Load bounds: ------------------------
# bnds = get_savedBounds_minmax("./bounds/AlexNet_bounds_ImageNet1000_act.txt")
bnds = get_savedBounds_minmax("./bounds/Resnet50_bounds_ImageNet_train20p_act.txt")
# bnds = get_savedBounds_minmax("./bounds/Vgg16_bounds_ImageNet_train20p_act.txt")
# print('Bounds used:', bnds)


# Note:
# - For ResNet, bounds can be extracted from original net or from the flattened net, in the end doesnt matter!
# - weight should be loaded for the original model, then model is copied including the weights! As long as no other layers with trainable parameters are added we are fine.
# - focuses on nn.ReLU here (but could be replaced by other act fcts)
# - forward pass should not have special operations, no skip connections that are not exactly the resnet bottlenecks
...
model = get_Ranger_protection(net, bnds)

# Compare:
# print('old:', list(net.children()))
# print('new:', list(model.children()))
...
tic = time.time()
with torch.no_grad():
    output = model(images) #model
    output_check = net(images)
    print(output)
    print(output_check)
    print('outputs match:', (np.array(output == output_check).all()))
toc = time.time()
...
# summary(model, torch.zeros(images.size()), show_hierarchical=True)


## Save to onnx

# # save ranger net
# onnx_file = './onnx/AlexNet_ranger_new' + ".onnx"
# # onnx_file = './onnx/ResNet50_ranger_new' + ".onnx"
# # onnx_file = './onnx/Vgg16_ranger_new' + ".onnx"
# # model = model.float()
# # with amp.disable_casts():
# save_torch_to_onnx(model, images[0].shape, onnx_file)
#
# # save original
# onnx_file = './onnx/AlexNet_new' + ".onnx"
# # onnx_file = './onnx/ResNet50_new' + ".onnx"
# # onnx_file = './onnx/Vgg16_new' + ".onnx"
# # net = model.float()
# # with amp.disable_casts():
# save_torch_to_onnx(net, images[0].shape, onnx_file)