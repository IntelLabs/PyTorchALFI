...
def main(argv):
    ...
    # yolov3_ErrorModel = TestErrorModels_ObjDet(model=yolov3, resil_model=yolov3_resil, model_name=model_name, dataset='ppp',\
    #     config_location='default_yolo.yml', dl_sampleN=0.125, dl_shuffle=False, device=device)
    ...
    fld_name = 'f27c7e7d-2d0a-4199-b685-ec99ff029d02_1_faults_[1]_bits'
    ground_truth_json_file_path = '/home/fgeissle/ranger_repo/ranger/result_files/yolov3_1_trials/weights_injs/per_image/' + fld_name + '/ppp/val/coco_format.json'
    detection_json_file_path = '/home/fgeissle/ranger_repo/ranger/result_files/yolov3_1_trials/weights_injs/per_image/' + fld_name + '/ppp/val/orig_model/epochs/0/coco_instances_results_0_epoch.json'
    corr_detection_json_file_path = '/home/fgeissle/ranger_repo/ranger/result_files/yolov3_1_trials/weights_injs/per_image/' + fld_name + '/ppp/val/corr_model/epochs/0/coco_instances_results_0_epoch.json'
    resil_detection_json_file_path = '/home/fgeissle/ranger_repo/ranger/result_files/yolov3_1_trials/weights_injs/per_image/' + fld_name + '/ppp/val/ranger_model/epochs/0/coco_instances_results_0_epoch.json'
    resil_corr_detection_json_file_path = '/home/fgeissle/ranger_repo/ranger/result_files/yolov3_1_trials/weights_injs/per_image/' + fld_name + '/ppp/val/ranger_corr_model/epochs/0/coco_instances_results_0_epoch.json'

    faults = read_fault_file('/home/fgeissle/ranger_repo/ranger/result_files/yolov3_1_trials/weights_injs/per_image/' + fld_name + '/ppp/val/yolov3_test_random_sbf_weights_inj_1_1_1bs_ppp_fault_locs.bin')

    ppp_dataloader = PPP_obj_det_dataloader(dataset_type='val', sampleN = 0.125) #do not need to register again if already done in TestErrorModels_ObjDet
    ...
    img_ids = [1, 2] #'all' #[1] #list [1,2,3] or 'all'


    # If desired, filter by score, manually
    thresh = 0.69

    # Check predictions vs gt
    annos_gt = visualizer.visualize_output(image_ids=img_ids, save_name='plots/ppp_test/gt/test_viz_gt_', json_file=ground_truth_json_file_path, thresh=None)
    annos_orig = visualizer.visualize_output(image_ids=img_ids, save_name='plots/ppp_test/orig/test_viz_orig_', json_file=detection_json_file_path, thresh=thresh)
    annos_corr = visualizer.visualize_output(image_ids=img_ids, save_name='plots/ppp_test/corr/test_viz_corr_', json_file=corr_detection_json_file_path, thresh=thresh)

    annos_orig_resil = visualizer.visualize_output(image_ids=img_ids, save_name='plots/ppp_test/orig_resil/test_viz_orig_resil_', json_file=resil_detection_json_file_path, thresh=thresh)
    annos_corr_resil = visualizer.visualize_output(image_ids=img_ids, save_name='plots/ppp_test/corr_resil/test_viz_corr_resil_', json_file=resil_corr_detection_json_file_path, thresh=thresh)
    print('Images saved')
    ...
    # ax[0].errorbar(np.arange(0,8+1), orig_av, yerr=orig_err, fmt='o')
    # ax[0].set_xlabel('bit position')
    # ax[0].set_ylabel('SDC rate - orig')
    # ax[0].set_ylim((0.2, 1))
    # ax[1].errorbar(np.arange(0,8+1), corr_av, yerr=corr_err, fmt='o')
    # ax[1].set_xlabel('bit position')
    # ax[1].set_ylabel('SDC rate - corr')
    # ax[1].set_ylim((0.2, 1))

    # fig.tight_layout()
    # plt.savefig("plots/evaluation/ppp_bpos_vs_sdc_2.png", dpi=300)
    # plt.show()
    ...
    # ax[0].errorbar(np.arange(0,72+1), orig_av, yerr=orig_err, fmt='o')
    # ax[0].set_xlabel('layer')
    # ax[0].set_ylabel('SDC rate - orig')
    # ax[0].set_ylim((0.2, 1.1))
    # ax[1].errorbar(np.arange(0,72+1), corr_av, yerr=corr_err, fmt='o')
    # ax[1].set_xlabel('layer')
    # ax[1].set_ylabel('SDC rate - corr')
    # ax[1].set_ylim((0.2, 1.1))
    # fig.tight_layout()

    # plt.savefig("plots/evaluation/ppp_lay_vs_sdc_2.png", dpi=300)
    # plt.show()
