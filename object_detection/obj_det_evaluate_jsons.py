...
def load_all_jsons(folder_path, epoch_nr):
...
def eval_epoch(epoch_nr, folder_path, iou_thresh, metadata, nan_infs, filter_ooI=True, eval_mode="iou+class_labels"):
    """
    ...
    coco_gt, coco_orig, coco_corr, coco_orig_resil, coco_corr_resil = load_all_jsons(folder_path, epoch_nr)
    ...
    for i in tqdm(range(len(coco_labels_grouped)), desc="progress:"): #i image counter
...
def eval_experiment(epochs, iou_thresh, folder_path, save_name, metadata, nan_infs, filter_ooI, eval_mode):
    """
    ...
    for epoch_nr in range(epochs):
        ...
        results_one_epoch, unannotated_images = eval_epoch(epoch_nr, folder_path, iou_thresh, metadata, nan_infs, filter_ooI, eval_mode=eval_mode) #corr nan_infs
