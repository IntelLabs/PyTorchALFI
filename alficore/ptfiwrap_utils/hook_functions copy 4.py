...
def set_ranger_hooks_v3(net, bnds, resil='ranger', mitigation=True, detector=False):
    """
    ...
    for _, m in net.named_modules():
        if type(m) in [torch.nn.ReLU, torch.nn.LeakyReLU, torch.nn.ELU, torch.nn.PReLU, torch.nn.Sigmoid, torch.nn.modules.activation.SiLU]:
            act_hook = Range_detector(bnds[cnt], act_list, quant_list, mitigation, detector)
            handle_out = m.register_forward_hook(act_hook)
            hook_handles_out.append(handle_out)
            hook_list.append(act_hook)
            cnt += 1

    return act_list, quant_list, hook_handles_out, hook_list

class Range_detector:
    def __init__(self, bnd, act_list, quant_list, mitigation, detector, check_perc=True):
        ...
        self.mit = mitigation
        self.det = detector #(if mit is off then nothing is saved since it is trivial)
        self.check_perc = check_perc
    ...
    def __call__(self, module, module_in, module_out):
        ...
        if self.det:
            ...
            self.act_list.append(torch.sum(comp>0, dim=[1,2,3]).tolist())

        if self.check_perc and len(self.bnd)>2:
            # check how quantiles behave
            tnsr = torch.flatten(module_out, start_dim=1, end_dim=- 1) #flatten all except for batch nr
            q25 = torch.quantile(tnsr, 0.25, dim=1).cpu().numpy()
            q50 = torch.quantile(tnsr, 0.50, dim=1).cpu().numpy()
            q75 = torch.quantile(tnsr, 0.75, dim=1).cpu().numpy()
            lst = np.vstack([q25, q50, q75])
            bnr = len(q25)
            comp = lst - np.array(self.bnd[1:-1]).repeat(bnr).reshape(3,bnr)
            # comp = np.array(self.bnd[1:-1]) - lst[:][1:-1]
            mse = np.sqrt(np.sum(comp**2, 0))
            self.quant_list.append(list(mse))

        if self.mit:
            return torch.clamp(module_out, min=self.bnd[0], max=self.bnd[-1])
        else:
            return module_out
