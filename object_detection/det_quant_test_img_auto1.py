...
def main(argv):
    ...
    ms = ['resnet', 'alexnet'] # 'resnet' #'alexnet', 'resnet', 'lenet'
    ds = ['imagenet', 'imagenet'] # 'imagenet' #'imagenet, mnist
    # ms = ['alexnet'] # 'resnet' #'alexnet', 'resnet', 'lenet'
    # ds = ['imagenet'] # 'imagenet' #'imagenet, mnist

    for a in range(len(ms)):
        # Define dataset and model specifications
        ####################################################################
        dataset_name = ds[a] #'imagenet' #'imagenet, mnist
        model_name = ms[a] #'resnet' #'alexnet', 'resnet', 'lenet'

        batch_size = 10 #batchsize for neurons
        num_faults = 1 #faults
        ...
        # Storing location ---------------------------------------------------------
        yml_file = 'default_min_quant_test_auto_img.yml'
        ...
        # #################################################################################
        target_list = []
        experiment_template = exp_class_img(dataset_name, model_name, yml_file_path, dl_attr, batch_size, model, yml_file, num_faults, quant_monitoring, ftrace_monitoring)
