...
""" Format:
    # Save results ---------------------------------------------------
    result_dict = dict()
    result_dict["orig_correct_pred"] = glob_correct_pred_runs.  # List structure: epoch -> batches -> images. 
    result_dict["fi_correct_pred"] = glob_fi_correct_pred_runs # format as above ...(for all 4 here)
    ...
    result_dict["protection"] = "Ranger" #TODO: choose
        ...
        if not my_dict_final: #if empty at first
            ...
            with open(file, 'rb') as f:
                ...
                if my_dict_final["fault_rate"] == new_dict["fault_rate"] and my_dict_final["injection_target"] == new_dict["injection_target"] \
                        ...
                        and my_dict_final["topN"] == new_dict["topN"] and my_dict_final["fixed_nr_faults"] == new_dict["fixed_nr_faults"] \
                    ...
                    for ep in range(len(my_dict_final["orig_correct_pred"])): #for each epoch combine batches
                        my_dict_final["orig_correct_pred"][ep] = my_dict_final["orig_correct_pred"][ep] + new_dict["orig_correct_pred"][ep]
                    for ep in range(len(my_dict_final["fi_correct_pred"])): #for each epoch combine batches
                        my_dict_final["fi_correct_pred"][ep] = my_dict_final["fi_correct_pred"][ep] + new_dict["fi_correct_pred"][ep]
                    ...
                    print(my_dict_final["topN"], new_dict["topN"])
...
def add_epochs_sameImages(my_dict_final, new_dict):
    ...
    if not my_dict_final: #if empty at first
        ...
        if my_dict_final["fault_rate"] == new_dict["fault_rate"] and my_dict_final["injection_target"] == new_dict["injection_target"] \
            ...
            my_dict_final["orig_correct_pred"] = my_dict_final["orig_correct_pred"] + new_dict["orig_correct_pred"]
            my_dict_final["fi_correct_pred"] = my_dict_final["fi_correct_pred"] + new_dict["fi_correct_pred"]
...
def analyse_folder(folder):
    ...
    dict_all["acc_unfiltered"] = [mean_epochs, std_epochs] #add to dict
    dict_all["fi_acc_unfiltered"] = [fi_mean_epochs, fi_std_epochs]

    # Filter by originally correct images
    ...
    # print('acc', 'mean', mean_epochs, 'std', std_epochs)
    # print('fi_acc', 'mean', fi_mean_epochs, 'std', fi_std_epochs)
    dict_all["acc_filtered"] = [mean_epochs, std_epochs] #add to dict
    dict_all["fi_acc_filtered"] = [fi_mean_epochs, fi_std_epochs]
    ...
    dict_all["sdc"] = [sdc_epochs, sdc_std_epochs] #add to dict
    dict_all["fi_sdc"] = [fi_sdc_epochs, fi_sdc_std_epochs]

    # # Get Ranger acts:
    # orig_ranger_flattrunc = [np.sign(j) for j in orig_ranger_flat] #multiple ranger per image become only one
    # fi_ranger_flattrunc = [np.sign(j) for j in fi_ranger_flat]
    # # orig_ranger_flattrunc = [np.sum(i) for i in orig_ranger_flattrunc] #how many images have activated ranger: rows runs, cols scalar
    # # fi_ranger_flattrunc = [np.sum(i) for i in fi_ranger_flattrunc]
    # # ranger_epochs, ranger_std_epochs, _ = get_acc_std_N(orig_ranger_flattrunc)
    # # fi_ranger_epochs, fi_ranger_std_epochs, _ = get_acc_std_N(fi_ranger_flattrunc)
    # # dict_all["ranger_act_nr"] = [ranger_epochs, ranger_std_epochs]  # add to dict
    # # dict_all["fi_ranger_act_nr"] = [fi_ranger_epochs, fi_ranger_std_epochs]
    # dict_all["ranger_act_nr"] = [np.sum(i) for i in orig_ranger_flattrunc]  # add to dict
    # dict_all["fi_ranger_act_nr"] = [np.sum(i) for i in fi_ranger_flattrunc] #todo check
