...
def get_nan_inf_masks(corr_due, dct_all, nr_samples, nr_all, typ='no_resil'):
    ...
    print('Ratio of images affected by due: ', np.sum(mask_nan_inf_flat)/nr_all, '(', np.sum(mask_nan_inf_flat), ' out of ', nr_all, ')')
    ...
    print('Ratio of images affected by sdc: ', np.sum(mask_any_change_and_no_naninf_flat)/nr_all, '(', np.sum(mask_any_change_and_no_naninf_flat), ' out of ', nr_all, ')')
...
def main(argv):
    ...
    """
    folder_paths = [
                    "result_files/yolov3_1_trials/neurons_injs/per_image/objDet_20221111-225432_1_faults_31_bits/coco"
                    ]
