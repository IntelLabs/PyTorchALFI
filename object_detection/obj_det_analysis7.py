...
def extract_sdc_due(json_path, flt_type, folder_path, faults_path, save_as_name, typ='no_ranger'):
    ...
    if faults_path is None:
        print("Warning: faults_path does not seem to exist.")
        sys.exit()
...
def get_nan_inf_masks(corr_due, dct_all, nr_samples, nr_all, typ='no_ranger'):
    ...
    print('Ratio of images affected by due: ', typ, np.sum(mask_nan_inf_flat)/nr_all, np.sum(mask_nan_inf_flat), nr_all)
    ...
    print('Ratio of images affected by sdc: ', typ, np.sum(mask_any_change_and_no_naninf_flat)/nr_all, np.sum(mask_any_change_and_no_naninf_flat), nr_all)
...
def main(argv):
    ...
    folder_path = '/nwstore/qutub/share_results/result_files_paper/retinanet_50_trails_ranger/retina_50_trials/weights_injs/per_batch/objDet_20211208-194836_1_faults_[0_32]_bits/coco2017/val'
    ...
    # # # (detectron2, kitti, no resil neurons, Y, extr, extr, extr, final)
    # folder_path = '/home/fgeissle/ranger_repo/ranger/result_files/det2_fasterRCNN_50_trials_Conv/weights_injs/objDet_20220215-150613_1_faults_[0, 31]_bits/kitti/val'
    # faults_path = get_fault_path(folder_path)
    # flt_type = "weights" #"weights"
    # suffix = '_none' #'_avg' # '_ranger'
    # model_name = "detectron" #yolov3 #retina
    ...
    json_file = extract_sdc_due(save_name, flt_type, folder_path, faults_path, save_as_name, typ='no_ranger')
