...
__all__ = ["load_robo_json", "load_sem_seg", "convert_to_coco_json", "register_coco_instances"]
...
def load_robo_json(json_file, image_root, split: float=0.0001, random: bool=False, dataset_name=None, extra_annotation_keys=None):
    """
    ...
    if split:
        if split <= 1:
            ...
            split = split/len(img_ids)
        val_split, _ = split_data(dataset_len=len(img_ids), split=split, random=random)
...
def register_robo_instances(name, metadata={}, json_file=None,\
    image_root=None, split:float=0.01, random:bool=False):
    """
    ...
    DatasetCatalog.register(name, lambda: load_robo_json(json_file, image_root, split, random, name)) #no split
...
class Robo_obj_det_dataloader:
    ...
    def __init__(self,  dl_attr:TEM_Dataloader_attr, dnn_model_name:str="yolov3_ultra"):
        ...
        **kwargs: Arbitrary keyword arguments.
        ...
        self.batch_size=self.dl_attr.dl_batch_size
        self.shuffle=self.dl_attr.dl_shuffle
        self.num_workers = self.dl_attr.dl_num_workers
        self.dataset_type = self.dl_attr.dl_dataset_type
        self.sampleN = self.dl_attr.dl_sampleN


        json_file = '/nwstore/norbert/dataset/humans_and_robots/labels/instances_robo2022.json' #'/nwstore/datasets/Ppp_test_data/json_format_labels/instances_ppp2021_xyxy_coco_labels.json'
        image_root = '/nwstore/norbert/dataset/humans_and_robots'
        if self.dataset_type == 'val':
            register_robo_instances(name = 'robo/{}'.format(self.dataset_type), metadata={}, json_file=json_file,\
        ...
        self.dataset_dict = DatasetCatalog.get('robo/{}'.format(self.dataset_type))
        self.metadata = MetadataCatalog.get('robo/{}'.format(self.dataset_type))
        ...
        print("Robo Obj Det Dataset loaded in {}; {} dataset - Length : {}".format(self.device, self.dataset_type, len(self.dataset)))
