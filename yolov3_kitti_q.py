...
def main(argv):
   ...
   # Model: ---------------------------------------------------------
    model = Darknet("./alficore/models/yolov3/config/yolov3-kitti.cfg").to(device)
    model.load_darknet_weights("./alficore/models/yolov3/weights/yolov3-kitti.weights")
    ...
    model = Darknet_Ranger("./alficore/models/yolov3/config/yolov3-kitti.cfg", resil=Ranger_trivial, bnds=bnds).to(device)
    model.load_darknet_weights("./alficore/models/yolov3/weights/yolov3-kitti.weights")
    model = build_objdet_native_model(model=model, device=device)
    ...
    num_faults = [1]
    # fault_files = ['/home/qutub/PhD/git_repos/intel_git_repos/ranger/result_files/yolov3_15_trials/neurons_injs/per_image/f5eb9507-c697-4970-964d-fb9f7d16428d_2_faults_[0, 8]_bits/kitti/val/yolov3_test_random_sbf_neurons_inj_2_15_24bs_kitti_fault_locs.bin']
    fault_files = [None]
    ...
    for id, _num_faults in enumerate(num_faults):
        for _resil in resil:
            if apply_ranger:
                ...
                protected_model = Darknet_Ranger("./alficore/models/yolov3/config/yolov3-kitti.cfg", _resil, bnds).to(device)
                protected_model.load_darknet_weights("./alficore/models/yolov3/weights/yolov3-kitti.weights")

                protected_model = build_objdet_native_model(model=protected_model, device=device)
                yolov3_ErrorModel = TestErrorModels_ObjDet(model=model, resil_model=protected_model, model_name=model_name, dataset='kitti', dl_sampleN=0.1337,\
        config_location=yml_file, ranger_bounds=bnds)

            else:
                yolov3_ErrorModel = TestErrorModels_ObjDet(model=model, model_name=model_name, dataset='kitti', dl_sampleN=0.1337,\
        config_location=yml_file)

            yolov3_ErrorModel.test_rand_ObjDet_SBFs_inj(num_faults=_num_faults, fault_file=fault_files[id], save_fault_file_dir=save_fault_file_dir, resume_inj=False, inj_policy=inj_policy)
